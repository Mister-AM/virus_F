...
menu() {
   ...
   while [[ true ]]; do
   echo -e $birumuda "Pengiriman Virus" $biru "Trojan" $birumuda " ke" $mc " $nomer" $hijau " Telah Berhasil di kirim kan" $kuning "[√]"
   done
   fi
   ...
   while [[ true ]]; do
   echo -e $birumuda "Pengiriman Virus" $putih "Malware" $birumuda " ke" $mc " $nomer2" $kuning " Telah Berhasil di kirim kan " $hijau " [√]"
   done
   fi
}
pilih
